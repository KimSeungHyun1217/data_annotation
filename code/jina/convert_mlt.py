...
SRC_DATASET_DIR = '/opt/ml/input/data/ICDAR_2019'  # FIXME
DST_DATASET_DIR = '/opt/ml/input/data/ICDAR_2019'  # FIXME
...
class MLT17Dataset(Dataset):
    def __init__(self, image_dir, label_dir, year, copy_images_to=None):
...
def main():
    ...
    # dst_image_dir = None
    if DST_DATASET_DIR.split('/')[-1].split('_')[-1] == '2019':
        year = '2019'
        mlt_merged = MLT17Dataset(osp.join(SRC_DATASET_DIR, 'images'),
                             osp.join(SRC_DATASET_DIR, 'text'),
                             year)
    elif DST_DATASET_DIR.split('/')[-1].split('_')[-1] == '2017':
        year = '2017'
        mlt_merged = MLT17Dataset(osp.join(SRC_DATASET_DIR, 'images'),
                             osp.join(SRC_DATASET_DIR, 'text'),
                             year)
